...
const CourseForm = ({ authors, onCancel }) => {
  ...
  const validateDescription = (description) =>
    description.length >= 4 && description.length <= 50;
  ...
  const validateForm = () => {
    ...
    if (!validateTitle(titleRef.current.value)) {
      newErrors.titleRef =
        "The title must contain between 4-20 alphabetical characters.";
    }
  ...
  const handleRemoveAuthor = (author) => {
    ...
    setAvailableAuthors(
      [...availableAuthors, author].sort((a, b) => {
        return (
          authors.findIndex((au) => au.id === a.id) -
          authors.findIndex((au) => au.id === b.id)
        );
      }),
    );
  };
      ...
      <form className="create-course__form" onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          ...
          <textarea
            ...
            required
          ></textarea>
          {errors.descriptionRef && (
            <div className="error">{errors.descriptionRef}</div>
          )}
        </div>
        ...
        <div className="form-group">
          ...
          />
          {errors.durationRef && (
            <div className="error">{errors.durationRef}</div>
          )}
        </div>
        ...
        <div className="authors">
            ...
            {availableAuthors.length > 0 ? (
              availableAuthors.map((author) => (
                <div className="authors__list-item" key={author.id}>
                  ...
                  <button type="button" onClick={() => handleAddAuthor(author)}>
                    Add
                  </button>
                </div>
              ))
          ...
          <div className="authors__new">
            ...
            />
            <button type="button" onClick={handleNewAuthor}>
              Create Author
            </button>
          </div>
        </div>
